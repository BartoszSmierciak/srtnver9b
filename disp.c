...
void button_record_clicked(void)
{
    ...
    if (d1.record) //jeśli nagrywanie było włączone 
    {
        d1.record = 0; //wyłącz nagrywanie 
        d1.foutstatus = 0;
        //przedefiniowanie koloru na czarny
        color.green = 0xffff;
        color.red = 0xffff;
        color.blue = 0xffff;
        //zmiana koloru przycisku na czarny
        gtk_widget_modify_bg(button_record, GTK_STATE_NORMAL, &color);
    } 
    else //jeśli nagrywanie wyłączone
    {
        d1.record = 1; //włacz nagrywanie 

        if (d1.entry6) //jeśli istnieje flaga wpisu entry6
            return; //zakończ funkcję
        entry6 = gtk_entry_new(); //jeśli nie istnieje utworz nowy wpis
        d1.entry6 = 1; //ustaw flagę entry6 na 1
