...
d1type d1;
double ras[NSOU], decs[NSOU], epoc[NSOU];//RA DEC EPOC , NSOU numer źródła
int soutype[NSOU]; //typ źrodła
char sounam[NSOU][25];//nazwa źródła
char soutrack[25], souinfo[25]; //śledzenie źródla
...
int main(int argc, char *argv[])
{
    ...
    sprintf(d1.catnam, "srt.cat"); // kopjuje plik srt.cat do zmiennej d1.catnam
    sprintf(d1.hlpnam, "srt.hlp"); //kopiuje plik srt.hlp do amiennej d1.hlpnam

    for (i = 0; i < argc - 1; i++) 
    {//pętla po argumentach wejściowych
        sscanf(argv[i], "%63s", buf); //skanowanie wartości argumentów i zapis do buf
        if (strstr(buf, "-c") && strlen(buf) == 2)//jeśli buf zawiera -c i ma długość 2 znaków
            sscanf(argv[i + 1], "%63s", d1.catnam); //to wczytaj kolejną wartośc argumentu i zapisz d1.catnam
        if (strstr(buf, "-h") && strlen(buf) == 2)//jeśli buf zawiera -h i ma długość 2 znaków
            sscanf(argv[i + 1], "%63s", d1.hlpnam);//to wczytaj kolejną wartośc argumentu i zapisz d1.hlpnam
    }
    //d1.azelport = 0x3f8;        // com1 default for old SRT 
    d1.ver = 6;                 // SRT software version
    d1.secs = readclock(); //aktualny czas
    d1.run = 1; // uruchamia pętlę programu, tylko ręczne?
    d1.record = 0; //flaga zapisu 0 brak zapiu, 1 zapis, zmiana w przycisku zapisu
    ...
    d1.bw = 2.4;                  // set to 2.4 for TV dongle 10 MHz for ADC card in init
    ...
    d1.azelsim = d1.radiosim = d1.fftsim = 0; //1- wlącza symulację radia, 0 wyłącza
    d1.mainten = 0;
    d1.stowatlim = 1;
    ...
    d1.south = 1;
    d1.hgt = 0;
    d1.dongle = 1;              // set to zero initially - set to 1 in Init_Device if dongle
    ...
    if (!catfile()) //wczytywanie parametrow z pliku srt.cat
        return 0;
    ...
    if (d1.lock) //zapisuje do pliku 1 lub 0 w zależności czy SRT jest uruchomiony czy nie
    {
        if ((file1 = fopen("lock.txt", "r")) == NULL) //w pliku tylko 0
        {
    ...
    //symulator obserwacji
    if (!d1.azelsim) //jeśli nie symulacja
    {
        if (d1.printout)
    ...
    else //jeśli nie symulacja obserwacji 
    {
        if (d1.stowatlim) 
        {
            d1.azprev = d1.azlim1;
