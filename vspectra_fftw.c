...
void vspectra(void)
{
    ...
    num = d1.nblk;//liczba bloków?              //was 20   // was 100
    nsam = NSAM; 
    d1.nsam = NSAM * num;//ilość sampli?
    avsig = 0;
    numm = 0;
    smax = 0;
    max = -1e99;
    min = 1e99;
    for (i = 0; i < blsiz2; i++) //blsix2 rozmiar bloku?
        vspec[i] = 0.0; //zerowanie tablicy
    for (k = 0; k < num; k++) 
    {
        if (!d1.radiosim)//odczyt dongla jeśli false
// Read the raw data from the RTL Dongle
            r = rtlsdr_read_sync(dev, bufferRead, nsam, &n_read);
        else //symulacja radia
        {
            av = 5.0;
            if (d1.elnow < 5.0)
