  ...
  // command antenna movement
{
        ...
        && (az >= d1.azlim1 && az < d1.azlim2 && el >= d1.ellim1 && el < d1.ellim2)) 
    {
        ...
        else 
        {
            ...
            if(!d1.south)
            {
                if(d1.azcmd < d1.azlim2 -360.0)
                    azz += 180.0; 
                if(d1.azprev < d1.azlim2 -360.0) 
                    azz += 180.0;   
                if(azz >= 360.0) 
                    azz += -360.0;
                if(azz >= 360.0) 
...
{ 
    ...
    {
        //  printf("here cmd = -1\n");
        system("stty -F /dev/ttyS0 2400 cs8 -cstopb -parenb -icanon min 1 time 20"); 
        usbdev = open("/dev/ttyS0",O_RDWR,O_NONBLOCK);
        sleep(1);
        status = close(usbdev);
    ...
    if(d1.stow == 1)
        d1.stow = d1.azcount = d1.elcount = 0;
    status = close(usbdev);
